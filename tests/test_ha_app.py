...
def test_addon_config_matches_supervisor_schema_expectations() -> None:
    ...
    assert re.fullmatch(r"2\.0\.0(?:-rc[1-9](?:-(?:talkdiag[12]|talkpace1|talkframe[12]))?)?", str(config["version"]))
    ...
    assert options["hacs_audio_mode"] == "aac"
    assert options["rtsp_backchannel_mode"] == "off"
    ...
    assert _enum_values(schema["hacs_audio_mode"]) == {"video_only", "aac"}
    assert _enum_values(schema["rtsp_backchannel_mode"]) == {
        "off", "onvif_require_audioback", "onvif_require_trackid2", "always_audioback"
    }
